...
mu1= [0.025 0.05 0.1 0.15 0.2 0.23 0.27 0.3 0.32 0.34 0.36 0.38 0.4] ;%
%mu1= [0.02:0.02:0.38] ;
atpCost.gam=35;
 atpCost.ngam=1;
 T1=[33 36 38];
...
[sol model complexFileName total_protein_molecules_model ]=modelGenerationGeko(MyModel,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('Std_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,active_translation);
%Sfactor= Estimate_abundance(model,mu, complexFileName,sprintf('Std_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-1.6',kk);
