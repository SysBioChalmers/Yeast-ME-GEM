function [sol model complexFileName molecule_number]=modelGenerationGECKO_Johan(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
molecule_number=floor(proteinRatio_Johan(mu)*cellweight/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
molecule_number_mu_max=200;%floor(proteinRatio_Johan(0.46)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
...
upr=-1.6667*mu + 0.76;
upr=0.22; %best parameter is 0.26
...
Metabolic_Sector=0*proteinRatio_Johan(mu);
Ribosome_Sector=0*proteinRatio_Johan(mu);
Engergy_Sector=0*Metabolic_Sector;
Maintaince_Sector=upr*proteinRatio_Johan(mu);
min_kcat=1;
kdeg=0.045;
kcat = 90*60*60;
kcat_for_missing=85;
...
f_rRNA=0.85;
kt=(1/0.22); %the best parameter is 0.22
r0=0.14;%the best parameter is 0.14
...
elseif strcmp(media,'Rich_AA')==1
    model=complex_medium(model,6);%SD media
else
...
%protein content constrains
fprintf(fptr,'CtotalProtein: %s = %f\n',ve_total,proteinRatio_Johan(mu)/(1-UnFoldRatio*(1-RefoldingRatio)));
...
%% Space constraints
fprintf(fptr,'CMIM: %s <= %f\n',membraneConditionAngelica(model, mu,kdeg,cellweight),c_mim);
