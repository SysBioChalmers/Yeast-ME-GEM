function [eq total_gram free_aa_ratio]= biomass_CEN_PK_temp(mu)
...
%    H2O= sprintf('%.15f H2O [cytoplasm]', H2O_ratio*1000/18.01528);
 ...
 %RNA
    RNA_ratio =(15.249*mu + 4.9841)/100;%(-14.883*mu*mu + 21.131*mu + 4.6712)/100;
...
% carbohydrates
    ...
    glucan_ratio=(540117*mu^6 - 745285*mu^5 + 401048*mu^4 - 104290*mu^3 + 12921*mu^2 - 611.55*mu + 19.633)/100-0.025;
    glycogen_ratio= (-581.6*mu^3 + 488.33*mu^2 - 148.74*mu + 18.095)/100-0.025;
    mannan_ratio= (-1.8223*mu + 10.367)/100-0.025;
    trehalose_ratio = (-43.601*mu + 9.2581)/100-0.025;
...
free_aa='';
   ...
   total_gram = total_gram + proteinRatio(mu);