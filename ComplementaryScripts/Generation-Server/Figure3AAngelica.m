...
mu1= [0.2 0.23 0.27 0.28 0.29 0.30 0.32 0.34] ;
atpCost.gam=43;
atpCost.ngam=1;
 T1=[33 36 38];
...
% 
 ...
 [sol model1 complexFileName total_protein_molecules_model ]=modelGenerationGECKO_AngelicaNew(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('Angelica_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,1);
 ...
 %[geneList PredictedProteinExpresstion(:,k) ribosomeRatio(k)] =modelProteinAllocation(model,sprintf('HAP4_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk),mu,0.045,'Soplex-1.6');
end
