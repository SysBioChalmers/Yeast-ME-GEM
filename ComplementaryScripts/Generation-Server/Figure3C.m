...
mu1= 0.3;%[0.025 0.05:0.05:0.35 0.38] ;
atpCost.gam=30;
 atpCost.ngam=1;
 T1=[33 36 38];
...
% 
 ...
 [sol model1 complexFileName total_protein_molecules_model ]=modelGenerationGECKO_u(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('tHAP4_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,1);
 Sfactor= Estimate_abundance(model,mu, complexFileName,sprintf('tHAP4_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-1.6',kk);
 [geneList PredictedProteinExpresstion(:,k) ribosomeRatio(k)] =modelProteinAllocation(model,sprintf('tHAP4_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk),mu,0.045,'Soplex-1.6');
end
...
system(sprintf('./myResults1 %s','tHAP4_Ribosome_Max_Mu_GEKO_Glucose'),'-echo');
% system('echo "Ribosome"; grep "X8079" *.out > ribosome_rzesult.txt','-echo');
