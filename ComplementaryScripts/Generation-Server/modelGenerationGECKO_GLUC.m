function [sol model complexFileName molecule_number]=modelGenerationGECKO_GLUC(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
mu=mu1; 
sigma_plasma = 1;
c_mim=50; %67 is the best parameter
...
for i=1:numel(complex_list) %check again for 1:n
    ...
    if (ks<=651)% strcmp(compartment(i),'mitochondrion')==0
