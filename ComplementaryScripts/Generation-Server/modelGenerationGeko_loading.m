function [sol model complexFileName molecule_number]=modelGenerationGeko(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
%          end        
        ...
        if numel(find(ismember(mitoProteins,peptide)))==0
            ...
            if strcmp(complex_list{i},'')==0 && report_reaction(i)==0
              ...
              %scape this 
                ...
                MW=0;
                    ...
                    elseif (numel(peptide)>1)  %complex is a hetrodimer.
                        I=find(ismember(subunit(:,1),model.grRules{i}));
              ...
              metabolic_rxn_index  = find(rxns_by_this_complex<=numel(model.grRules));
              rxns_by_this_complex= rxns_by_this_complex(model.grRules);
              ...
              elseif rxn_kcat<=min_kcat
               ...
               for r=2:numel(rxns_by_this_complex)
                ...
                if numel(find(ismember(model.grRules(i),Protein_plasma(:,1))))>=1
                    
                   if numel(find(ismember(glcouse_transporter_genes, cell2mat(model.grRules(i)))))>0
                       if mu<=0.27
            ...
            fprintf(fptr_sfactor,'S%d\t%s\t%s\n',ks,complex_list{i},strrep(model.grRules{i},' ','_'));
            if ks<=numel(S_factor(:))
                    SS= S_factor(ks);
                    if SS==0
                        SS=sigma;
