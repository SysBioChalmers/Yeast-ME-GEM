...
mu=mu1; 
sigma_plasma = 1;
c_mim=78;%67; %67 is the best parameter

average_amino_acids=126;
...
mitochondrionVolume=0.88;%0.88;%Ratio of mitochondrion to cell volume is equal 1-2%
cyto_volume =4;
plasma_area=60;
plasma_budjet=10;
Vmax_NH4 = 7;
...
f_rRNA=0.85;
kt=(1/0.35); %the best parameter is 0.22
...
%translation factors
sTF=0.8;
kcat_ribo_scanning = 0.01*sTF*kcat_ribo*60*60*sigmaTemp;
...
%Assembly Factors
kcat_assembly=(1/2000)*2000*60*sigmaTemp;
kdeg_asembly= kdeg;
...
elseif strcmp(media,'infinity')==1
    model=complex_medium(model,7);%infinity media
else
...
fprintf(fptr,'PMRcarbon:   %s <= %f\n',plasmaConditionCarbon(model, mu,kdeg),0.1*plasma_area);
...
%two lines.
   ...
   fprintf(fptr,'CCV1: %s  - XXCV1 =  0  \n',constrain1);
   fprintf(fptr,'CCV2: %s  + XXCV1 - XXCV2  =  0  \n',constrain2);
   fprintf(fptr,'CCV3: %s  + XXCV2 - XXCV3 =0\n \n',constrain3);
   fprintf(fptr,'CCV4: XXCV3 <= %f\n',cyto_volume);
...
N_cpc=2e6;
N_cpc=N_cpc*(mu+kdeg)*(1/(13*6.023e8));
kk=1;
for i=1:numel(model.rxns)
