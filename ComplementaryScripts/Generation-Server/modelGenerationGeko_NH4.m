function [sol model complexFileName molecule_number]=modelGenerationGeko_NH4(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
molecule_number=floor(proteinRatioNitrogen(mu)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
molecule_number_mu_max=floor(proteinRatioNitrogen(0.46)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 

mitochondrionVolume=0.8*2/3;%Ratio of mitochondrion to cell volume is equal 1-2%
cyto_volume =4;
plasma_area=40;
plasma_budjet=10;
Vmax_NH4 =2*7.7*mu/2.6*0.75;
...
Metabolic_Sector=0*proteinRatioNitrogen(mu);
Ribosome_Sector=0*proteinRatioNitrogen(mu);
Engergy_Sector=0*Metabolic_Sector;
Maintaince_Sector=upr*proteinRatioNitrogen(mu);
min_kcat=1;
kdeg=0.045;
kcat = 90*60*60;
kcat_for_missing=90;
...
%kcat_rate = c_ribosome * .29/0.077 * (mu + kdeg);
...
if strcmp(objective,'min_NH4')
    fprintf(fptr,'obj: - X4172\n');
elseif strcmp(objective,'min_glucose')==1
...
elseif strcmp(objective,'max_unmodelled_protein')==1
    ...
    fprintf(fptr,'obj: X%d\n',index_obj);
elseif strcmp(objective,'min_translation')==1
    index_obj=find(ismember(model.rxns,'Ribosome_biosynthesis'));
    fprintf(fptr,'obj: -X%d\n',index_obj);
            ...
            if strcmp(complex_list{i},'')==0 && report_reaction(i)==0
              ...
              elseif rxn_kcat<=min_kcat
               ...
               for r=2:numel(rxns_by_this_complex)
                ...
                NH4_transporter_genes={'YDR384C' 'YGR121C' 'YNL142W' 'YPR138C'};
                ...
                if numel(find(ismember(complex_list(i),Protein_plasma(:,1))))>=1
                    if numel(find(ismember(NH4_transporter_genes, cell2mat(complex_list(i)))))>0
                        if mu==0.1
                          saturation_transporter= 0.004;%(0.171*mu - 0.0142)/sigma;
                        elseif mu<=0.2
                          saturation_transporter= (0.171*mu - 0.0142)/sigma;
                        else
                          saturation_transporter=1;
                        end
                   ...
                   if numel(find(ismember(glcouse_transporter_genes, cell2mat(complex_list(i)))))>0
                       if mu==0.1
                        saturation_transporter=0.007;%0.75/sigma;
                       else
                        saturation_transporter=1;%0.75/sigma;
                       end
...
%protein content constrains
fprintf(fptr,'CtotalProtein: %s = %f\n',ve_total,proteinRatioNitrogen(mu)/(1-UnFoldRatio*(1-RefoldingRatio)));
