...
% model=addYeastReaction(model,eq,{rxnID},{rxnNames},0,100,0,{''});
 ...
 [sol model1 complexFileName total_protein_molecules_model ]=modelGenerationGECKO(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('test_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,1);
%    Sfactor= Estimate_abundance(model,mu, complexFileName,sprintf('Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-3.0',kk);
 [geneList PredictedProteinExpresstion(:,k) ribosomeRatio(k)] =modelProteinAllocation(model,sprintf('test_Ribosome_Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk),mu,0.045,'Soplex-1.6');
end
...
system(sprintf('./myResults1 %s','test_Ribosome_Max_Mu_GEKO_Glucose'),'-echo');
