...
mu1= [0.2 .23 0.27  0.30 0.32 0.34] ;
mu1= [0.027 0.044 0.1023 0.152, 0.2143 0.254, 0.284, 0.333, 0.376] ;
mu_exp= [0.025 0.05 0.10 0.15 0.2 0.25 0.3  0.35, 0.4] ;
mu1= [0.2 .23 0.27 0.29  0.30 0.32 0.34 0.35 0.37] ;
mu1=0.27:0.01:0.37;
atpCost.gam=33;
atpCost.ngam=1;
 T1=[33 36 38];
...
PredictedProteinMass=zeros(836,numel(mu1));
...
% 
 ...
 ExpressionFileName=sprintf('growth_rate_%.2g.xlsx',mu1(k));
 [sol model1 complexFileName total_protein_molecules_model ]=modelGenerationGECKO(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('JProteome_comparsion_GEKCO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,1);
 %Sfactor= Estimate_abundance(model,mu, complexFileName,sprintf('JProteome_comparsion_GEKCO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-1.6',kk);
 %PredictedProteinMass(:,k) = Estimate_abundance(model,mu, complexFileName,sprintf('JProteome_comparsion_GEKCO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-1.6',kk);
 [geneList PredictedProteinExpresstion(:,k) ribosomeRatio(k)] =modelProteinAllocation(model,sprintf('JProteome_comparsion_GEKCO_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk),mu,0.045,'Soplex-1.6');
end
...
system(sprintf('./myResults1 %s','JProteome_comparsion_GEKCO_Glucose_'),'-echo');
