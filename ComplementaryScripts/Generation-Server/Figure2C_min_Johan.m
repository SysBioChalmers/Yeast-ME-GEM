...
[sol{i} glc  ]=growthSearchDynamic_Johan(new_MyModel, 'minimum',atpCost,0,T, copies, overExpressed, complex_list,compartment,0.3, 0.45,fileName,'Soplex-1.6',1)
[ss{i} model1 complexFileName ]=modelGenerationGECKO_Johan(new_MyModel,'minimum', sol{i}.mu-0.01,atpCost,'min_carbon',0,T,0, overExpressed, fileName,complex_list,compartment,'Soplex-1.6',1,1);
...
for i=MM:NN
    fprintf('%s\t%f\t%f\t%f\t%f\t%.15g\t%f\t%f\n',cell2mat(c(i,1)),sol{i}.mu,sol{i}.mu-0.01,ss{i}.X(4321),ss{i}.X(4321),sol{i}.X(8079),sol{i}.X(4259),sol{i}.X(4321));
end
save Batch_min
