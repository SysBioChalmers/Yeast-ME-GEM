function [sol model complexFileName molecule_number]=modelGenerationGECKO_Jianye(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
mu=mu1; 
sigma_plasma = 1;
c_mim=85; %67 is the best parameter
...
upr=-1.6667*mu + 0.76;
upr=0.16; %best parameter is 0.26
...
for i=1:numel(complex_list) %check again for 1:n
    ...
    if (ks<=90000)% strcmp(compartment(i),'mitochondrion')==0
        ...
        if numel(find(ismember(mitoProteins,peptide)))==0
            ...
            if strcmp(complex_list{i},'')==0 && report_reaction(i)==0
              ...
              if (length(find(ignore==ks))>=1)
               SS=1;
              end  
               end
%               if (length(find([622 633]==ks))>=1)
%                   SS=3;
%               end
              % SS=1;
              %scape this 
