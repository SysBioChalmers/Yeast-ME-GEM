function  Sfactor= Estimate_abundanceMW(model,growth_rate, complexFileName,outputLPFile, ExpressionFileName,total_protein_molecules_model,SoplexVersion,Iteration)
...
%    ModelExpression(i)=sol.X(rxn_index)/growth_rate*(13*6.023e8);
   ...
   ModelExpression(i)=sol.X(rxn_index)/growth_rate*MW/1000;
   ...
   abundance=0;
      ...
      for j=1:numel(peptide)
          ...
          if k==0 %no subunit is in the data
              ...
              expression(i)=abundance/k*nsubunits/(13*6.023e8)*MW/1000; %the expression is the aveage of the subunits
          end
