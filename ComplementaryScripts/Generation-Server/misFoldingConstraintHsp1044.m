function misFoldingConstraintHsp1044(model,fptr,tempFactor,mu,kcat,kdeg,sigmaTemp,RefoldingRatio,T)
[a b Proteins_length]=xlsread('TableS1.xlsx','Length');
[a b native_ratio]=xlsread('20180214_deltaG_and_f_at_different_T_deltaT.xls');
...
if (r>=23044)&&(r<=23078)
    ...
    if numel(name)>0
        ...
        c1 = mu/(kdeg+mu)*unfolded_ratio*(1-RefoldingRatio);
        c2 = kdeg/(kdeg + mu)*unfolded_ratio*(1-RefoldingRatio);
        c3 = kf /(mu +kdeg)*unfolded_ratio*RefoldingRatio;
        
        rxn_misfold=strrep(cell2mat(model.rxns(r)),'folding_cytoplasm','translation');
