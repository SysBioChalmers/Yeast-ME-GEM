...
eq=sprintf(' => L-glutamate [extracellular]');
rxnID=sprintf('glutamate_source');
rxnNames=rxnID;
MyModel=addYeastReaction(MyModel,eq,{rxnID},{rxnNames},3,3,0,{''}); %cell2mat(metabolite(i,3))

% %close ammonium importer
MyModel.lb(4172)=0;
MyModel.ub(4172)=0;
...
[sol model1 complexFileName total_protein_molecules_model ]=modelGenerationGECKO_GLUC(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('GLU_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-3.0',kk,1);
 %Sfactor= Estimate_abundance(model,mu, complexFileName,sprintf('GLU_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk), ExpressionFileName,total_protein_molecules_model,'Soplex-1.6',kk);
 %[geneList PredictedProteinExpresstion(:,k) ribosomeRatio(k)] =modelProteinAllocation(model,sprintf('GLU_Glucose_%f_T%d_Copies_%d_k_%d.lp.out',mu,T,0,kk),mu,0.045,'Soplex-1.6');
end
...
system(sprintf('./myResults1 %s','GLU_Glucose'),'-echo');
