function [sol model complexFileName molecule_number]=modelGenerationGeko_CBS(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
molecule_number=floor(proteinRatioCBS(mu)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
molecule_number_mu_max=floor(proteinRatioCBS(0.46)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 

mitochondrionVolume=0.20*.13*46;%Ratio of mitochondrion to cell volume is equal 1-2%
cyto_volume =4;
plasma_area=50;
plasma_budjet=10;
Vmax_NH4 = 7;
...
Metabolic_Sector=0*proteinRatioCBS(mu);
Ribosome_Sector=0*proteinRatioCBS(mu);
Engergy_Sector=0*Metabolic_Sector;
Maintaince_Sector=upr*proteinRatioCBS(mu);
min_kcat=1;
kdeg=0.048;
kcat = 90*60*60;
kcat_for_missing=80;
...
%protein content constrains
fprintf(fptr,'CtotalProtein: %s = %f\n',ve_total,proteinRatioCBS(mu)/(1-UnFoldRatio*(1-RefoldingRatio)));
