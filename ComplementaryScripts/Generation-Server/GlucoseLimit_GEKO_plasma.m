...
mu1= 0.3%[0.24 0.27 0.28 0.29 0.3 0.32 0.34 0.36 0.38];%[0.25:0.01:0.4]% 0.32 0.34 0.35 0.36 0.38 0.4] ;%
%mu1= [0.02:0.02:0.38] ;
atpCost.gam=30;
 atpCost.ngam=1;
 T1=[33 36 38];
...
% 
% model.ub(find(ismember(model.rxns,'r_1126_reverse')))=0;
...
% model=addYeastReaction(model,eq,{rxnID},{rxnNames},0,10,0,{''});



   ExpressionFileName=sprintf('growth_rate_%.2g.xlsx',mu);
   [sol model complexFileName total_protein_molecules_model ]=modelGenerationGeko_plasma(model,'minimum', mu,atpCost,'min_glucose',22.6,T,0, overExpressed, sprintf('Max_Mu_GEKO_Glucose_%f_T%d_Copies_%d_k_%d.lp',mu,T,0,kk),complex_list,compartment,'Soplex-1.6',kk,1);
