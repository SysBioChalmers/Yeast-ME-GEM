...
fileName=sprintf('%s_batch_min.lp',cell2mat(c(i,1)));
...
[sol{i} glc  ]=growthSearchDynamic(new_MyModel, 'minimum',atpCost,0,T, copies, overExpressed, complex_list,compartment,0, 0.6,fileName,'Soplex-1.6',1)
...
fileName=sprintf('%s_MIN_batch.lp',cell2mat(c(i,1)));


[ss model1 complexFileName ]=modelGenerationGeko(new_MyModel,'minimum', sol{i}.mu,atpCost,'min_carbon',0,T,0, overExpressed, fileName,complex_list,compartment,'Soplex-1.6',1,1);
[geneList PredictedProteinExpresstion ribosomeRatio] =modelProteinAllocation(model,sprintf('%s.out',fileName),sol{i}.mu,0.045,'Soplex-1.6');
...
for i=1:NN
    fprintf('%s\t%f\t%.15g\n',cell2mat(c(i,1)),sol{i}.mu,ss.X(8079));
end
save MIN_Figure1D