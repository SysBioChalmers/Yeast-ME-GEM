function [sol model complexFileName molecule_number]=modelGenerationGeko_glycerol(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
mitochondrionVolume=0.6;%Ratio of mitochondrion to cell volume is equal 1-2%
cyto_volume =4;
plasma_area=50;
plasma_budjet=10;
Vmax_NH4 = 7;
               ...
               for r=2:numel(rxns_by_this_complex)
                ...
                if numel(find(ismember(complex_list(i),Protein_plasma(:,1))))>=1
                   ...
                   if numel(find(ismember(glcouse_transporter_genes, cell2mat(complex_list(i)))))>0
                       if mu<=0.27
                        saturation_transporter=0.28*mu + 0.1355;%0.75/sigma;
                       else
                        saturation_transporter= 2.3027*mu - 0.4047;%0.75/sigma;
                       end
