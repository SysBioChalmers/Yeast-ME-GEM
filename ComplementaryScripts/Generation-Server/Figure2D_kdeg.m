...
fileName=sprintf('%s_batch_all_min.lp',cell2mat(c(i,1)));
...
[sol{i} glc  ]=growthSearchDynamic_kdeg(new_MyModel, 'minimum',atpCost,0,T, copies, overExpressed, complex_list,compartment,0.0, 0.6,fileName,'Soplex-1.6',1)
[ss{i} model1 complexFileName ]=modelGenerationGECKO_kdeg(new_MyModel,'minimum', sol{i}.mu,atpCost,'min_carbon',0,T,0, overExpressed, fileName,complex_list,compartment,'Soplex-1.6',1,1);
...
for i=MM:NN
    fprintf('%s\t%f\t%.15g\t%.15g\n',cell2mat(c(i,1)),sol{i}.mu,ss{i}.X(8079),  ss{i}.X(4321));
end
save Batch_min_all
