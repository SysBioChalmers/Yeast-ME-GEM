...
% riboflavin
 ...
 % carbohydrates
    ...
    glucan_ratio=(540117*mu^6 - 745285*mu^5 + 401048*mu^4 - 104290*mu^3 + 12921*mu^2 - 611.55*mu + 19.633)/100+0.05;
    glycogen_ratio= (-581.6*mu^3 + 488.33*mu^2 - 148.74*mu + 18.095)/100+0.025;
    mannan_ratio= (-1.8223*mu + 10.367)/100+0.05;
    trehalose_ratio = (-43.601*mu + 9.2581)/100;
    if trehalose_ratio <0
        trehalose_ratio =0.025;
    end
          deference= (1- (proteinRatio_Johan(mu) + total_gram + glucan_ratio + glycogen_ratio + mannan_ratio + trehalose_ratio));
          if deference<0
...
free_aa_ratio =1-(proteinRatio_Johan(mu) + total_gram);
