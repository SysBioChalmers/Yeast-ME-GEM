function [sol model complexFileName molecule_number]=modelGenerationGeko_Angelaca(MyModel,media,mu1,atpCost,objective,glucoseUptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,Iteration,active_translation)
...
molecule_number=floor(proteinRatio_Angelica(mu)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
molecule_number_mu_max=floor(proteinRatio_Angelica(0.46)*13/(average_protein_length*average_amino_acids)*6.023e23/1e12); 
...
Metabolic_Sector=0*proteinRatio_Angelica(mu);
Ribosome_Sector=0*proteinRatio_Angelica(mu);
Engergy_Sector=0*Metabolic_Sector;
Maintaince_Sector=upr*proteinRatio_Angelica(mu);
min_kcat=1;
kdeg=0.045;
kcat = 90*60*60;
kcat_for_missing=80;
...
kcat_ribo_eEF1A =sTF*kcat_ribo*60*60*sigmaTemp;
kcat_ribo_eEF1B =sTF*kcat_ribo*60*60*sigmaTemp;
kcat_ribo_eEF2 =sTF*kcat_ribo*60*60*sigmaTemp;
kcat_ribo_eEF3 =sTF*kcat_ribo*60*sigmaTemp;
kcat_ribo_eRF=0.17*60*60*sigmaTemp;

Mito_ribosome_active_ratio=1;
kcat_ribo_mito= 20*60*60;
kdeg_ribo_mito= kdeg;
...
%Assembly Factors
kcat_assembly=(1/5)*2000*60*sigmaTemp;
kdeg_asembly= kdeg;
...
for i=1:numel(complex_list) %check again for 1:n
    ...
    if (ks<=900)% strcmp(compartment(i),'mitochondrion')==0
        ...
        if numel(find(ismember(mitoProteins,peptide)))==0
            ...
            if strcmp(complex_list{i},'')==0 && report_reaction(i)==0
              ...
              if Iteration ==1
                  ...
                  UnChanged_Set=[715 807 819   835 831];
              end
              if numel(find(UnChanged_Set==ks))>0
                  SS=30;
              end
...
%protein content constrains
fprintf(fptr,'CtotalProtein: %s = %f\n',ve_total,proteinRatio_Angelica(mu)/(1-UnFoldRatio*(1-RefoldingRatio)));
