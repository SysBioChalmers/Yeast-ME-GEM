function [sol1 glc]= growthSearchDynamic_mutant(model, medium,atpCost,uptake,T, copies, overExpressed, complex_list,compartment,gr_min, gr_max,fileName,SoplexVersion,active_translation)
...
while (abs(gr_max - gr_min)>=1e-3)
  ...
  sol=modelGenerationGeko_mutant(model,medium,gr,atpCost,'no_objective',uptake,T,copies, overExpressed, fileName,complex_list,compartment,SoplexVersion,1,active_translation);
