function model=addCCT(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'TRIC/CCT'));
I1=find(ismember(proteins(:,1),'TRIC/CCT'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => CCT [cytoplasm]',eq);
rxnID=sprintf('CCT_biosynthesis');

rxnName=sprintf('biosynthesis of CCT');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('CCT [cytoplasm] => %s',eq_deg);
rxnID=sprintf('CCT_degradation');

rxnName=sprintf('Degradation of CCT');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('CCT [cytoplasm] => ');
rxnID=sprintf('CCT_dilution');

rxnName=sprintf('Dilution of CCT');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
