function newModel= addProteinDegradation2ModelRatio(newModel,n,m)
...
eq=sprintf('%d H2O [cytoplasm] + %d ATP [cytoplasm] + %s_peptide [cytoplasm] => %s + %d ADP [cytoplasm] + %d phosphate [cytoplasm] + %d H+ [cytoplasm]',nATP, nATP,gene,eq_product,nATP,nATP,nATP);

rxnID=sprintf('%s_subunit_degradation_ratio',gene);
 rxnID=strrep(rxnID,'-','');
