function  [eq_product number_of_aa]= degrade(seq)
...
for i=1:20
    if (aa_count(i) >0)
        ...
        if (strcmp(eq_product,''))
            eq_product= sprintf('%d %s [cytoplasm]',aa_count(i),cell2mat(aa_names(i)));
        else
            eq_product= sprintf('%s + %d %s [cytoplasm]',eq_product,aa_count(i),cell2mat(aa_names(i)));
        end
    end
