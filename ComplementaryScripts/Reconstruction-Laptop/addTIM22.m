function model=addTIM22(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'TIM Complex'));
I1=find(ismember(proteins(:,1),'Small Tim complexs'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => TIM22 [mitochondrion]',eq);
rxnID=sprintf('TIM22_Complex_biosynthesis');

rxnName=sprintf('biosynthesis of TIM22 Complex');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('TIM22 [mitochondrion] => %s',eq_deg);
rxnID=sprintf('TIM22_degradation');

rxnName=sprintf('Degradation of TIM22');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('TIM22 [mitochondrion] => ');
rxnID=sprintf('TIM22_dilution');

rxnName=sprintf('Dilution of TIM22');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
