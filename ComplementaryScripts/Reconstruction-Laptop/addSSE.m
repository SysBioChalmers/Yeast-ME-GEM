function model=addSSE(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'SSE'));
I1=find(ismember(proteins(:,1),'SSE'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => SSE [cytoplasm]',eq);
rxnID=sprintf('SSE_biosynthesis');

rxnName=sprintf('biosynthesis of SSE');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('SSE [cytoplasm] => %s',eq_deg);
rxnID=sprintf('SSE_degradation');

rxnName=sprintf('Degradation of SSE');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('SSE [cytoplasm] => ');
rxnID=sprintf('SSE_dilution');

rxnName=sprintf('Dilution of SSE');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
