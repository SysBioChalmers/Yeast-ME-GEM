function model=addHSP40(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'HSP40'));
I1=find(ismember(proteins(:,1),'HSP40'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => HSP40 [cytoplasm]',eq);
rxnID=sprintf('HSP40_biosynthesis');

rxnName=sprintf('biosynthesis of HSP40');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('HSP40 [cytoplasm] => %s',eq_deg);
rxnID=sprintf('HSP40_degradation');

rxnName=sprintf('Degradation of HSP40');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('HSP40 [cytoplasm] => ');
rxnID=sprintf('HSP40_dilution');

rxnName=sprintf('Dilution of HSP40');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
