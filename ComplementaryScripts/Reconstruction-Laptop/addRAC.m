function model=addRAC(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'RAC'));
I1=find(ismember(proteins(:,1),'RAC'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => RAC [cytoplasm]',eq);
rxnID=sprintf('RAC_biosynthesis');

rxnName=sprintf('biosynthesis of RAC');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('RAC [cytoplasm] => %s',eq_deg);
rxnID=sprintf('RAC_degradation');

rxnName=sprintf('Degradation of RAC');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('RAC [cytoplasm] => ');
rxnID=sprintf('RAC_dilution');

rxnName=sprintf('Dilution of RAC');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
