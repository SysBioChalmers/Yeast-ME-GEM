function model=addSSB(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'SSB'));
I1=find(ismember(proteins(:,1),'SSB'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => SSB [cytoplasm]',eq);
rxnID=sprintf('SSB_biosynthesis');

rxnName=sprintf('biosynthesis of SSB');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('SSB [cytoplasm] => %s',eq_deg);
rxnID=sprintf('SSB_degradation');

rxnName=sprintf('Degradation of SSB');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('SSB [cytoplasm] => ');
rxnID=sprintf('SSB_dilution');

rxnName=sprintf('Dilution of SSB');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
