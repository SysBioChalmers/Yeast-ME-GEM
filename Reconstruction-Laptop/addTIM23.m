...
eq = sprintf('%s => TIM23 [mitochondrion]',eq);
rxnID=sprintf('TIM23_Complex_biosynthesis');

rxnName=sprintf('biosynthesis of TIM23 Complex');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('TIM23 [mitochondrion] => %s',eq_deg);
rxnID=sprintf('TIM23_degradation');

rxnName=sprintf('Degradation of TIM23');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('TIM23 [mitochondrion] => ');
rxnID=sprintf('TIM23_dilution');

rxnName=sprintf('Dilution of TIM23');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
