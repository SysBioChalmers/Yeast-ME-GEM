clc
 ...
 [n a tRNAsequence]=xlsread('tRNA_modification_position.xlsx','Sequence');
 [n a Position]=xlsread('tRNA_modification_position.xlsx','Position');
 [n a Reaction]=xlsread('tRNA_modification_position.xlsx','Reaction');
...
for i=2:43 % loop for tRNA
   ...
   [eq modification_base]=tRNAreactions(base,N,Reaction,Position);
   ...
   [eq modification_base]=tRNAreactions(base,C,Reaction,Position);
   ...
   if cell2mat(tRNAsequence(i,10))==1
       ...
       syn_codon=regexp(cell2mat(tRNAsequence(i,16)),';','split');
       for kk=1:numel(syn_codon)
           fprintf(fptr,'\t%s-tRNA(%s) => %s-tRNA(%s)\n',aa,codon,aa,cell2mat(syn_codon(kk)));
           fprintf(fptr,'\ttRNA(%s) => %s%s\n',cell2mat(syn_codon(kk)),modification_base,cytosol_compartment);
           %fprintf(fptr1,'\t%s-tRNA(%s) => %s-tRNA(%s)\n',aa,codon,aa,cell2mat(syn_codon(kk)));
           fprintf(fptr1,'\t%s-tRNA(%s)\t%s-tRNA(%s)\n',aa,codon,aa,cell2mat(syn_codon(kk)));
