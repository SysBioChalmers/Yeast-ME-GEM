function model=addSSA(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'SSA'));
I1=find(ismember(proteins(:,1),'SSA'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => SSA [cytoplasm]',eq);
rxnID=sprintf('SSA_biosynthesis');

rxnName=sprintf('biosynthesis of SSA');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('SSA [cytoplasm] => %s',eq_deg);
rxnID=sprintf('SSA_degradation');

rxnName=sprintf('Degradation of SSA');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('SSA [cytoplasm] => ');
rxnID=sprintf('SSA_dilution');

rxnName=sprintf('Dilution of SSA');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
