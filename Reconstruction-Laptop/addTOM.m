function model=addTOM(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'Tom Complex'));
I1=find(ismember(proteins(:,1),'Tom Complex'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => TOM [mitochondrion]',eq);
rxnID=sprintf('Tom_Complex_biosynthesis');

rxnName=sprintf('biosynthesis of Tom Complex');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('TOM [mitochondrion] => %s',eq_deg);
rxnID=sprintf('TOM_degradation');

rxnName=sprintf('Degradation of TOM');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('TOM [mitochondrion] => ');
rxnID=sprintf('TOM_dilution');

rxnName=sprintf('Dilution of TOM');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
