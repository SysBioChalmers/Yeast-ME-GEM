function model=addHSP90(model)
[a b proteins]=xlsread('TableS1.xlsx','Annotation');
I=find(ismember(proteins(:,1),'HSP90'));
I1=find(ismember(proteins(:,1),'HSP90'));
n=min(I);
m=max(I);
...
eq = sprintf('%s => HSP90 [cytoplasm]',eq);
rxnID=sprintf('HSP90_biosynthesis');

rxnName=sprintf('biosynthesis of HSP90');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add digradtion
eq = sprintf('HSP90 [cytoplasm] => %s',eq_deg);
rxnID=sprintf('HSP90_degradation');

rxnName=sprintf('Degradation of HSP90');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});

% add dilution
eq = sprintf('HSP90 [cytoplasm] => ');
rxnID=sprintf('HSP90_dilution');

rxnName=sprintf('Dilution of HSP90');
model=addYeastReaction(model,eq,{rxnID},{rxnName},0,1000,0,{''});
