...
for i=2:43 % loop for tRNA
   ...
   eq=sprintf('%d ATP [nucleus] + %d CTP [nucleus] + %d GTP [nucleus] + %d UTP [nucleus] => tRNA_%s_%s_transcript [nucleus] + %d diphosphate [nucleus]',bc.A , bc.C , bc.G , bc.T,aa,codon,(bc.A + bc.C + bc.G + bc.T));
   base=sprintf('tRNA_%s_%s_transcript',cell2mat(tRNAsequence(i,3)),cell2mat(tRNAsequence(i,4)));
  
   rxnID=sprintf('tRNA_%s_%s_transcription',aa,codon);
   rxnNames=sprintf('tRNA_%s_%s transcription',aa,codon);
   model=addYeastReaction(model,eq,{rxnID},{rxnNames},0,100,0,{''});
      
   [rxnID rxnNames eq modification_base]=tRNAreactions(base,N,Reaction,Position);
   ...
   if cell2mat(tRNAsequence(i,11))==1
       ...
       if strcmp(RS_gene,'YDR023W;YHR011W')==0
           ...
           eq=sprintf('\t%s%s + H2O [cytoplasm] + ATP [cytoplasm] + %s%s => AMP [cytoplasm] + diphosphate [cytoplasm] + %s-tRNA(%s)%s\n',modification_base,cytosol_compartment,AA,cytosol_compartment,aa,codon,cytosol_compartment);
           eq=strrep(eq,'[c]',' [cytoplasm]');
       ...
       syn_codon=regexp(cell2mat(tRNAsequence(i,5)),';','split');
       for kk=1:numel(syn_codon)
               eq=sprintf('\t%s-tRNA(%s) [cytoplasm] => %s-tRNA(%s) [cytoplasm]\n',aa,codon,aa,cell2mat(syn_codon(kk)))
               rxnID=sprintf('codon_synonym_%s_%s',codon,cell2mat(syn_codon(kk)))
               rxnNames=sprintf('codon %s has %s synonym',codon,cell2mat(syn_codon(kk)))
               model=addYeastReaction(model,eq,{rxnID},{rxnNames},0,100,0,{''});
               
               eq=sprintf('tRNA(%s) [cytoplasm] => %s_splicing [cytoplasm]\n',cell2mat(syn_codon(kk)),modification_base);
               rxnID=sprintf('codon_synonym_%s_%s_relase',codon,cell2mat(syn_codon(kk)))
               rxnNames=sprintf('codon %s has %s synonym relase',codon,cell2mat(syn_codon(kk)))
               model=addYeastReaction(model,eq,{rxnID},{rxnNames},0,100,0,{''});
               
               
               fprintf(fptr1,'%s-tRNA(%s) [cytoplasm]\t%s_splicing [cytoplasm]\n',aa,cell2mat(syn_codon(kk)),modification_base);
